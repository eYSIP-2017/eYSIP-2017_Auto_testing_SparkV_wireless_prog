			...
			for line in file1:
				...
				if re.search('test_main_1',line):
					...
					if (bool(re.search('PASS',line))):
						...
						totalmarks += 10
						flag = 1
				elif re.search('test_main_3',line):
					if (bool(re.search('PASS',line))):
						k+=1
						file2.write('%d. Toggling of LED, 2.5 Marks\n' % k)
						k+=1
			...
			if flag == 0:
				...
				for line in file1:
					...
					if (bool(re.search('test_led_on_topfour',line))):
						if (bool(re.search('PASS',line))):
							k+=1
							file2.write('%d. Top four LEDs Turn on, 2.5 Marks\n' % k)
							totalmarks += 2.5
						else:
							k+=1
							file2.write('%d. Top four LEDs Turn on, 0 Marks\n' % k)
					if (bool(re.search('test_led_on_bottomfour',line))):
						if (bool(re.search('PASS',line))):
							k+=1
							file2.write('%d. Bottom four LEDs Turn on, 2.5 Marks\n' % k)
							totalmarks += 2.5
						else:
							k+=1
