#This is the script that is to be run for automatic evaluation and compilation of submissions.
	...
	for file_name in list_of_files:	
		...
		with open(list_of_files[i], 'r') as file1:
			...
			if re.search("#include \"lcd.h\"", line):
				...
				file2.write(line)
				file2.write("\nint check[4];\nint a;")	# Adding an Array to the Submission to store multiple writes to a single variable
					...
					if  bool(re.search('FAIL',line)):
						...
						for line in file1:
							if re.search('timer1_init',line):
								...
								if re.search('PASS',line):	
									k+=1
									file2.write('%d. Counter Values Displayed, 4 Marks\n' % k)
									totalmarks += 4
								else:	
									k+=1
									file2.write('%d. Counter Values Displayed, 0 Marks\n' % k)
								...
								if re.search('PASS',line):	
									k+=1
									file2.write('%d. Buzzer turned on, 3 Marks\n' % k)
									totalmarks += 3
								else:	
									k+=1
									file2.write('%d. Buzzer turned on, 0 Marks\n' % k)
