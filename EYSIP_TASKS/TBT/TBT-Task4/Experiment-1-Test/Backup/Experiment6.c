...
void timer1_init(void)
{
	...
	TCCR1B = 	0x05;//Timer 1 mode selection
	TCNT1H =  	0x8F; //Counter higher 8 bit value
	TCNT1L = 	0x80; //Counter lower 8 bit value
	TCCR1B = 	0x05;//start Timer by selecting prescalar (avoid overwriting mode selection bits)
}
...
ISR(TIMER1_OVF_vect)
{
	...
	counter = counter+2;    // increment counter variable every time, the overflow occurs .
...
int main(void)
{
	...
	timer1_init();
	lcd_print(2,1,00,2);   // display initial value of the counter variable by providing appropriate arguments to this function.
	...
	while(1)
	{
		...
		//You can turn off the timer by disabling its clock source.
		// Turn on the buzzer for 1 second to indicate end of the count. 
		// You can use _delay_ms() for turning on the buzzer for 1 second.
