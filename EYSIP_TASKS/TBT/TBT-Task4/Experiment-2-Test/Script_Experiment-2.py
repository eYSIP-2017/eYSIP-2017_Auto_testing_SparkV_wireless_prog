...
path_to_mocked_function_header = "../../../../../CMock/lib/mocks/MockTBT4.h"	# Path to the Mocked Function header file

path_to_mocked_function_c = "../../../../CMock/lib/mocks/MockTBT4.c"	# Path to the Mocked Function C file
...
def add_required_syntax():
	...
	for file_name in list_of_files:	
		...
		with open(list_of_files[i], 'r') as file1:
			...
			elif re.search("PORTC =", line):
				file2.write(line);
				file2.write("check[a] = PORTC;\na++;\n")  # When PORTC is written to, the written value is stored in the Array.
...
def add_ISR_call():
    """Adds a function to call the required ISR, in order to test it""" 	  	
	i = 0 
	list_of_temp = sorted(glob.glob('./Temp/*.c'))
	for file_name in list_of_files:	
		file2 = open(list_of_temp[i],'a')
		with open(list_of_files[i], 'r') as file1:
			file2.write("void isr_call(void)\n{\n  __vector_20();\n}")  # The function__vector_20 calls the ISR for Timer 1 
			i += 1

def rename_main(): 
    """Searches the submissions for the Main function and renames it accordingly"""
	i = 0 
	list_of_temp = sorted(glob.glob('./Temp/*.c'))
	for file_name in list_of_files:	
		file2 = open(list_of_temp[i],'w+')
	...
	with open("Makefile", "r+") as file1:
		for line in file1:	
			if re.search("Compile:", line):
				 file2.write(line)
				 for file_name in list_of_files:				
					file2.write("	gcc -Os -DTEST -std=c99 " + list_of_files[i] + " Experiment_2_test.c " + path_to_mocked_function_c+ " " + path_to_untiy + " -o ./Test" +str(i)+ ".elf" +"\n")
				 	i += 1  
			else:
				 file2.write(line)
	file2.close()
						...
						for line in file1:
							if re.search('timer1_init',line):
								if re.search('PASS',line):	
									k+=1
									file2.write('%d. Initilization of Timer, 3 Marks\n' % k)
									totalmarks += 3
								else:	
									k+=1
									file2.write('%d. Initilization of Timer, 0 Marks\n' % k)
							elif re.search('test_ISR',line):
								if re.search('PASS',line):	
									k+=1
									file2.write('%d. Counter Values Displayed, 3 Marks\n' % k)
									totalmarks += 3
								else:	
									k+=1
									file2.write('%d. Counter Values Displayed, 0 Marks\n' % k)
							elif re.search('test_main',line):
								if re.search('PASS',line):	
									k+=1
									file2.write('%d. Buzzer, 4 Marks\n' % k)
									totalmarks += 4
								else:	
									k+=1
									file2.write('%d. Buzzer, 0 Marks\n' % k)
...
def final_grades():
	"""Creates a final Excell Sheet with the required headings"""
	 ...
	 temp.write("Team ID, CPU Frequency, Port Initialization, Timer initalization, Buzzer, Counter Values Displayed, Total Marks \n" )
	 temp.close
	 import pandas as pd
