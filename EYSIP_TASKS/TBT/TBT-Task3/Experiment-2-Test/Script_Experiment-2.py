...
path_to_mocked_delay_function_header = "../../../../../CMock/lib/mocks/MockTBT3.h"
path_to_mocked_delay_function_c = "../../../../CMock/lib/mocks/MockTBT3.c"
...
def assign_grades(): # function is used to assign grades to the submissions based on the Unit Test 	
	...
	for file_name in list_of_files:
		...
		with open('./Test_Results/Submission' + str(i+1) + '.txt', 'r') as file1:
			for line in file1:
				if bool(re.search('port_config',line)):
					if  bool(re.search('FAIL',line)):
						k+=1
						file2.write('%d. Ports initalization, 0 Marks\n' % k)
						k+=1
						file2.write('%d. Conversion, 0 Marks\n' % k)
						k+=1
						file2.write('%d. Converted Values Displayed, 0 Marks\n' % k)
					else:
						k+=1
						file2.write('%d. Ports initalization, 3 Marks\n' % k)
						totalmarks += 3
						for line in file1:
							if re.search('test_function',line):
								if re.search('PASS',line):	
									k+=1
									file2.write('%d. Conversion, 4 Marks\n' % k)
									totalmarks += 4
								else:	
									k+=1
									file2.write('%d. Conversion, 0 Marks\n' % k)
								...
								if re.search('PASS',line):	
									k+=1
									file2.write('%d. Converted Values Displayed, 3 Marks\n' % k)
									totalmarks += 3
								else:	
									k+=1
									file2.write('%d. Converted Values Displayed, 0 Marks\n' % k)
...
def final_grades():	   
	 ...
	 temp.write("Team ID, CPU Frequency, Port Initialization, Displayed at required position, Displayed Message,Total Marks \n" )
	 temp.close
	 import pandas as pd
