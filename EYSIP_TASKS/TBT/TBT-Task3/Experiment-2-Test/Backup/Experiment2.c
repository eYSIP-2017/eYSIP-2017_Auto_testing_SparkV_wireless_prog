...
#include "lcd.h"
#include "lcd.c"		// user-defined header file - it is included in project folder
...
if(number==a)
     {
	digit_tens= 53 ;            //Step 1: extract the ten's digit using the division operator and convert to its ascii equivalent
	lcd_wr_char(53);    	 //Step 2: pass the ten's digit as argument to lcd_wr_char()

	//Similarly write code for unit's digit 
	//Note cursor is set to auto increment so no need to set cursor every time

	digit_units= 52 ;			//Step 3: extract the unit's digit using the modulo operator and convert to its ascii equivalent
	lcd_wr_char(52);
	}
	else
	{
		digit_tens= 54 ;            //Step 1: extract the ten's digit using the division operator and convert to its ascii equivalent
	lcd_wr_char(54);    	 //Step 2: pass the ten's digit as argument to lcd_wr_char()

	//Similarly write code for unit's digit 
	//Note cursor is set to auto increment so no need to set cursor every time

	digit_units= 51 ;			//Step 3: extract the unit's digit using the modulo operator and convert to its ascii equivalent
	lcd_wr_char(51);
	 }      	//Step 4: pass the unit's digit as argument to lcd_wr_char()
}
