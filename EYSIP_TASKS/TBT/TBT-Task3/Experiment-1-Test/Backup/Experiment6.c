...
int main (void)
{
	...
	lcd_wr_command(0xC0);	//Write command to move the cursor to 2nd line 1st column. Cursor will be set at this position.
	//Refer to the written tutorial for all the lcd commands.
	
	lcd_wr_char('e'); // here along with appropriate arguments to display the required text
	lcd_wr_char('-');
	lcd_wr_char('Y');
	lcd_wr_char('a');
	lcd_wr_char('n');
