	...
	for file_name in list_of_files:
		...
		with open(file_name, 'r') as file1:
			...
			if '#define F_CPU' in line:
				if '14745600' in line:
					temp.write('1. CPU Frequency, 0 Mark\n') 
					totalmarks += 1
					temp.close
				else:
					temp.write('1. CPU Frequency, 0 Mark\n')
...
def add_required_syntax(): # function is used to add the required syntax to make the given Embedded Program testable
	...
	for file_name in list_of_files:	
		...
		with open(list_of_files[i], 'r') as file1:
			...
			elif re.search("F_CPU", line):
				file2.write(line)
				file2.write("\nint check_bar_led[9];\nint check_velocity_left[10];\nint check_velocity_right[10];\n")
				file2.write("int a; int b;\n")	
			elif bool(re.search("PORTJ =", line)):
				file2.write(line);
				file2.write("check_bar_led[a] = PORTJ;\na++;\n")
			elif bool(re.search("PORTJ=", line)):
				file2.write(line);
				file2.write("check_bar_led[a] = PORTJ;\na++;\n")	
			elif bool(re.search("OCR5BL =", line)):
				file2.write(line);
				file2.write("check_velocity_left[b] = OCR5AL;\ncheck_velocity_right[b] = OCR5BL;\nb++;\n")
			elif bool(re.search("OCR5BL=", line)):
				file2.write(line);
				file2.write("check_velocity_left[b] = OCR5AL;\ncheck_velocity_right[b] = OCR5BL;\nb++;\n")			
			else:
	...
	with open("Makefile", "r+") as file1:
		for line in file1:	
			if re.search("Compile:", line):
				 file2.write(line)
				 for file_name in list_of_files:				
					file2.write("	gcc -Os -DTEST -std=c99 " + list_of_files[i] + " Experiment-2-test.c " + path_to_mocked_function_c+ " " + path_to_untiy + " -o ./Test" +str(i)+ ".elf" +"\n")
				 	i += 1  
			else:
				 file2.write(line)
	file2.close()
...
def assign_grades(): # function is used to assign grades to the submissions based on the Unit Test 
	...
	for file_name in list_of_files:
		...
		with open('./Test_Results/Submission' + str(i+1) + '.txt', 'r') as file1:
			for line in file1:
				if bool(re.search('motion_pin_config',line)):
					if  bool(re.search('FAIL',line)):
						k+=1
						file2.write('%d. Port initalization, 0 Marks\n' % k)
						k+=1
						file2.write('%d. Decelerating the robot, 0 Marks\n' % k)
						k+=1
						file2.write('%d. LEDs being turned off, 0 Marks\n' % k)
					else:
						k+=1
						file2.write('%d. Port initalization, 4 Marks\n' % k)
						totalmarks += 4
						with open('./Test_Results/Submission' + str(i+1) + '.txt', 'r') as file3:
							for line in file3:
								print 'k'
								if re.search('test_velocity',line):
									if re.search('PASS',line):	
										k+=1
										file2.write('%d. Decelerating the robot, 4 Marks\n' % k)
										totalmarks += 4
									else:	
										k+=1
										file2.write('%d. Decelerating the robot, 0 Marks\n' % k)
								if re.search('test_LED',line):
									if re.search('PASS',line):	
										k+=1
										file2.write('%d. LEDs being turned off, 4 Marks\n' % k)
										totalmarks += 2
									else:	
										k+=1
										file2.write('%d. LEDs being turned off, 0 Marks\n' % k)
...
def final_grades():
	 ...
	 temp.write("Team ID, CPU Frequency, Port initalization, Decelerating the robot, LEDs being turned off, Total Marks \n" )
	 temp.close
	 import pandas as pd
