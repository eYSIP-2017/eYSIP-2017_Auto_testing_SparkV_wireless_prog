...
{
	TCCR5B = 0x08;	//Stop the timer initially
...
void forward (void) 
{
 ...
 //define the port to rotate both wheels forward
}
/*
* Function Name: stop 
* Input: none
...
{
  PORTA= PORTA & 0xF0;
 //define the port to stop the motor
}
/*
...
void motion_barLED()
{
	int i,l=255,r=255;  //start with max velocity
	for(i=0;i<=8;i++)	//on each iteration velocity is decremented and 
	{					//also the led bar graph height is decrementing
		forward();		// forward motion
		velocity(l,r);	// fuction call for changing the velocity
		_delay_ms(2000);// 2seconds delay for each movement
		l-=25;r-=25;	// velocity decremented by 25;
		PORTJ = PORTJ >> 1; // bar  graph level changing by shfting right
	}
