	...
	for file_name in list_of_files:	
		...
		with open(list_of_files[i], 'r') as file1:
			...
			if re.search("#include \"lcd.h\"", line):
				...
				file2.write(line)
				file2.write("\nint motion_check[12];\nint a;\n")	
			elif bool(re.search("PORTA =", line)):
				file2.write(line);
				file2.write("motion_check[a] = PORTA;\na++;\n")
			elif bool(re.search("PORTA=", line)):
				file2.write(line);
				file2.write("motion_check[a] = PORTA;\na++;\n")				
			else:
			...
			for line in file1:
				if bool(re.search('motion_pin_config',line)):
					if  bool(re.search('FAIL',line)):
						...
						k+=1
						file2.write('%d. Shape Formation, 0 Marks\n' % k)
					else:
						k+=1
						...
						for line in file1:
							if re.search('test_main',line):
								if re.search('PASS',line):	
									k+=1
									file2.write('%d. Shape Formation, 6 Marks\n' % k)
									totalmarks += 6
								else:	
									k+=1
									file2.write('%d. Shape Formation, 0 Marks\n' % k)
...
def final_grades():
	 ...
	 temp.write("Team ID, CPU Frequency, Port initalization, Shape Formation, Total Marks \n" )
	 temp.close
	 import pandas as pd
